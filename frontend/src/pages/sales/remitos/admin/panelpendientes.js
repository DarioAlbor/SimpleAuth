...
import { FaAngleDown, FaTrash, FaSave} from 'react-icons/fa';
...
const Panel = () => {
  ...
  const handleSaveEstado = async (remitoId) => {
    try {
      // Llamar a la función para editar el estado del remito
      const response = await axios.put(`http://localhost:3001/api/remitos/editar/${remitoId}`, {
        estado: selectedEstado,
      });

      console.log('Respuesta del servidor al editar el estado del remito:', response.data);

      // Actualizar la lista de remitos después de la edición
      cargarRemitos();
    } catch (error) {
      console.error('Error al editar el estado del remito:', error);
    }
  };
                ...
                <Tr>
                  ...
                  <Td>
                    ...
                    <IconButton
                      ...
                      icon={<Icon as={FaSave} />}
                      onClick={() => handleSaveEstado(firstRemitoWithNumber.id)}
                      variant="ghost"
                      colorScheme="green"
                    />
                  </Td>
