...
export default function SetAvatar() {
  ...
  useEffect(async () => {
    if (!localStorage.getItem(process.env.REACT_APP_45.162.169.217_KEY))
      navigate("/login");
  }, []);
  ...
  const setProfilePicture = async () => {
    if (selectedAvatar === undefined) {
      ...
      const user = await JSON.parse(
        localStorage.getItem(process.env.REACT_APP_45.162.169.217_KEY)
      );
      ...
      if (data.isSet) {
        ...
        localStorage.setItem(
          process.env.REACT_APP_45.162.169.217_KEY,
          JSON.stringify(user)
        );
        navigate("/");
      } else {
