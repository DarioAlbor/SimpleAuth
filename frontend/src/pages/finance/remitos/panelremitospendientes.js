...
const Panel = () => {
  ...
  const handleDeleteRemito = async (nroRemito) => {
    try {
      // Confirmar si el usuario realmente quiere eliminar el remito
      ...
      // Llamar a la función para eliminar el remito
  ...
  const handleSaveEstado = async (nroRemito) => {
    try {
      ...
      if (remitosAprobados.length === 0) {
        // No hay remitos aprobados para cambiar a 'Pagado'
        return;
      }
