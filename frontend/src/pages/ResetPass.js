...
const ResetPass = () => {
    ...
    useEffect(() => {
        ...
        const validateToken = async () => {
            try {
                const response = await axios.post('http://portal.drogueriagarzon.com/apilogin/validate-reset-token', { token });
    ...
    const handleResetPassClick = async (newPassword) => {
        ...
        try {
            const response = await axios.post('http://portal.drogueriagarzon.com/apilogin/reset-password', {
                email,
                token,
                newPassword,
            });
