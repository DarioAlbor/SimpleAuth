...
const ResetPass = () => {
    ...
    useEffect(() => {
        ...
        const validateToken = async () => {
            try {
                const response = await axios.post('https://portal.drogueriagarzon.com/api/login/validate-reset-token', { token });
    ...
    const handleResetPassClick = async (newPassword) => {
        ...
        try {
            const response = await axios.post('https://portal.drogueriagarzon.com/api/login/reset-password', {
                email,
                token,
                newPassword,
            });
