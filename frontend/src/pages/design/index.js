...
const DesignPage = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (file && /\.(jpe?g|png|gif)$/i.test(file.name)) {
            ...
            try {
                const response = await fetch('https://portal.drogueriagarzon.com:3001/api/uploadbanner', {
                    method: 'POST',
                    credentials: 'include',
                    body: formData,
                });
    ...
    const handlePDFChange = async (file) => {
        ...
        try {
            const response = await fetch('https://portal.drogueriagarzon.com:3001/api/uploadpdf', {
                method: 'POST',
                credentials: 'include',
                body: formData,
            });
